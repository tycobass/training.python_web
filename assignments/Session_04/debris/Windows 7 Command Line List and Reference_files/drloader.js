...
if($ezJQuery(".testimonial_slider").length>0){this.drWidgets.push('testimonial');this.log('Common Check: adding testimonial fix');}};this.loadDrWidgets=function(){if(this.drWidgets.length>0){for(var i=0;i<this.drWidgets.length;i++){var widget=this.drWidgets[i];this.log(widget);this.runSingleCheck(widget);}}};this.loadWidget=function(widget){if(widget!==undefined&&$lcl.inArray(widget,this.loadedWidgets)<0){$widgetURL=this.options.widgetBaseURL+ widget+'.js?dirname='+ this.options.dn;if($widgetURL.indexOf("?")>0){$widgetURL=$widgetURL+'&cb='+ this.options.cb;}
...
return arr;};$lcl.ezoicDoctor.defaultOptions={widgets:[],debugOutput:'0',widgetBaseURL:'/utilcave_com/dr/',dn:''};$lcl.fn.ezoicDoctor=function(options){return this.each(function(){(new $lcl.ezoicDoctor($lcl(this),options));});};}($ezJQuery));